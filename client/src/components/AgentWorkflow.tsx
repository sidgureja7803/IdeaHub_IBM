        ...
        <div className="relative mx-auto max-w-4xl overflow-hidden">
          ...
          <div className="lg:hidden space-y-3">
            {agents.map((agent, index) => (
              <motion.div
                key={index}
                className={`rounded-lg border ${activeAgent === index
              ...
              >
                <div
                  className={`p-4 flex items-center gap-3 cursor-pointer ${activeAgent === index ? `bg-${agent.color.split('-')[1]}-50 dark:bg-${agent.color.split('-')[1]}-900/20` : ''
                    }`}
                  onClick={() => setActiveAgent(index)}
                >
